...
def confidence_interval(x):
    ...
    st_dev = np.std(x, ddof = 1) #sample standard deviation, not you need to account for the standard devation of the sample
    ...
    results = {'UCL':UCL, 'sample_mean':samp_mean, 'LCL':LCL} #compile to a dictionary
...
def sampling_eng(x, n_samples = 100000):
    ...
    results_sd = np.std(results, ddof = 1) #calculate the standard deviation of the resamples
    UCL = results_mu + (2 * results_sd) #two standard deviations up
    LCL = results_mu - (2 * results_sd) #two standard deviations down
    results_dict = {'UCL':UCL, 'sample_mean':samp_mean, 'LCL':LCL} #compile to a dictionary
